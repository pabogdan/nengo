...
def test_sqrt_beta_analytical(n, m, seed):
    """Tests pdf, cdf, and invcdf of SqrtBeta distribution."""
    ...
    # The invcdf should give back x
    assert np.allclose(x, invcdf, atol=0.01)


@pytest.mark.parametrize("d", [2, 3, 10, 50])
def test_cosine_similarity(d, seed):
    ...
    exp = exp_dist.sample(num_samples, d)[:, 0]
    act = act_dist.sample(num_samples)
...
def test_cosine_analytical(d):
    ...
    assert np.allclose(dist.invcdf(cdf_exp), x, atol=0.01)


@pytest.mark.parametrize("d,p", [(3, 0), (5, 0.4), (10, 0.7), (50, 1.0)])
def test_cosine_intercept(d, p, seed):
